...
public class FileManagerWindow extends JFrame implements TreeSelectionListener, ISearchComplete, WindowListener {
	...
	JCheckBoxMenuItem options_useUnluac;
	...
	private void setupMenu(){
		...
		options_useUnluac = new JCheckBoxMenuItem(Strings.MENUITEM_USE_UNLUAC, false);
		options_useUnluac.setActionCommand("options_useUnluac");
		options_useUnluac.setState(true);
		...
		else if (contentType == 4 || file.getName().endsWith("atex"))
		{
			...
			if (luadec != null)	{
				try{
					...
					{
						luaView = new Lua_View(("-- Decompiled using luadec 2.0.1 by sztupy (http://winmo.sztupy.hu)\n"+luadec.decompile(data)).split("\n"));
					}
